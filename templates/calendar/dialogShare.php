<div class="dialog-parent invisible" id="dialog-share">
    <div class="dialog card pt-5 shadow-sm">
        <h1 class="text-center mb-3">
            Share
        </h1>
        <div class="desktop-only card-body">
            <div class="contacts card list-group shadow-sm w-100 mb-3 email-list"></div>
            <label>
                E-Mail
            </label>
            <div class="d-flex justify-content-between">
                <input type="email" class="mb-2 mr-2 w-100 email">
                <button type="button" class=" add-email btn btn-secondary mb-2">
                    Add
                </button>
            </div>
            <button type="button" class="remove-email btn btn-secondary">
                Remove
            </button>
            <button type="button" class="btn btn-primary toggleShare">
                Close
            </button>
        </div>
        <div class="mobile-only card-body">
            <div class="contacts card list-group shadow-sm w-100 mb-3 email-list"></div>
            <label>
                E-Mail
            </label>
            <input type="email" class="mb-2 mr-2 w-100 email">
            <button type="button" class="add-email btn btn-secondary w-100 mb-4">
                Add
            </button>
            <button type="button" class="remove-email btn btn-secondary mb-4 w-100">
                Remove
            </button>
            <button type="button" class="btn btn-primary toggleShare w-100">
                Close
            </button>
        </div>
    </div>
</div>